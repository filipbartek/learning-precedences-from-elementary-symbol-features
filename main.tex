...
\end{align*}
\todo{Martin: MSE neni presne, kdyz nakonec pouzijeme Lasso/Ridge. Mozna vynechat?}
by linear regression.
\todo[author=Martin]{Since we use Lasso, we don't minimize MSE.}
...
% Witness: PUZ001-1.p
an indicator of being produced during clausification and, % introduced
an indicator of being produced during skolemization. % skolem
...
\todo{Martin: Co zavest znaceni pro feature-vectorizaci symbolu? $\mathit{fv}$ se mi mnohokrat hodilo pri uvahach na tabuli! }
...
\begin{itemize}
	\item the input: the embedding of a symbol pair \(s_i, s_j\) in problem \(P\),
	\todo{Martin: tady by pak bylo presnejsi pouzi $\mathit{fv}$}
	\item the target: \({W_P}_{i,j}\) -- an element of the preference matrix we learned for problem \(P\)
	corresponding to the symbols pair \((s_i,s_j)\).
\end{itemize}
...
\todo{Martin: Co zavest znaceni pro vysledny nauceny model? Neco jako $\mathit{reg2}$?}
...
\begin{enumerate}
	...
	\todo{Martin: trochu se mi nelibi, ze se tady opakuje \(W_P\). Drive to byly naucene preference 
	z linearniho klasifikatoru pro trenovaci problem. Ted ji stavime z $\mathit{reg2}$ pro testovaci problem!  }
...
\todo{Martin: tohle by mohlo/melo byt presnejsi: \({W_P}_{i,j} =  \mathit{reg2}([\mathit{fv}(s_i),\mathit{fv}(s_j)])\) }

At this moment, one can use \(W_P\) to estimate the cost of an arbitrary symbol precedence.
% To uz se moc opakujeme:
...
Since this task is NP-hard in general,  \todo{Martin: tady by to chtelo taky citaci! Mozna jenom \cite{Cohen2011}? }
...
Martin: To by mohlo byt zajimave, ale momentalne mame asi dulezitejsi veci v hledacku.}
...
\todo{Martin: tohle se rika jeste jednou dole, ale tam uz je to presnejsi, protoze uz ctenar vi,
co je $\potential(s_i)$ a muzes rict: the smallest potential.
Asi bych se pokusit to preformulovat tak, abys to rikal jenom jednou a uz presne.}
...
To evaluate the stability of training, we repeat the experiment 5 times with different seeds.
Each time we sample a training set of 1000 problems and test set of 1000 problems.
...
We further restrict the training set based on the results of 24 probing proof attempts
with uniformly random predicate precedences:
we only consider problems with at least 1 successful proof search out of the 24.
% problems/predicate-solvable.txt
...
and a strategy fixed up to the predicate precedence.\footnote{Time limit: 10 seconds, memory limit: 8192 MB, literal comparison mode: predicate, function symbol precedence: \texttt{invfreq}, saturation algorithm: discount, age-weight ratio: 10, avatar: disabled.}
...
After we fit a preference matrix on each of the training problems (see \autoref{sect:preferences}),
...
\begin{tabular}{lrr}
	...
	\hline
	Best of 10 uniformly random precedences & 501.80 & 13.85 \\
	Machine learning with ElasticNetCV & 476.60 & 12.67 \\
	Vampire with \texttt{invfreq} & 475.80 & 13.83 \\
	Random precedence & 450.40 & 10.25 \\
\end{tabular}
% Source: https://docs.google.com/spreadsheets/d/1HSsC7piUAtWt6uwA9SYOX5vmiF0Ab3Ae4FPyGsDALIg/edit?usp=sharing
\todo[inline,author=Filip]{Add results from the experiment with weighting on and off.}

Our machine learning model on average matches the performance
