...
In~\autoref{sect:preferences} we formalize this idea using the notion of \emph{pairwise symbol preferences}
and show how, for each problem in isolation, these preferences can be learned using linear regression.
...
That is why we resort to representing symbols by their \emph{features} (cf.~\autoref{sect:embeddings})
when aggregating the learned preferences across different problems.
This is in more details explained further below and, formally, in \autoref{sect:general}.
...
problems. To obtain commensurable values, we normalise (see \autoref{sec:precedence-valuation}) the prover performance data 
on a per problem basis before learning the preferences. Normalisation also deals with
...
It should be stressed that while the learning of the preferences \emph{requires} a linear regression model by design,
this second-order regression does not need to be linear and more sophisticated models can be experimented with. 

The details of this step are given in \autoref{sec:general-preference}.
...
such that $\pi_P$ orders the symbol $s_i$ before the symbol $s_j$ -- see \autoref{sect:construction} for details).
...
The algorithm is recalled in \autoref{sect:ltot}.
...
whose mutual order does not matter for the performance.
...
In the following sections we assume that the preference matrix \(W_P\) we find by linear regression
...
we find a problem preference matrix by the method outlined in \autoref{sect:preferences}.
...
symbol arity, % arity
number of symbol occurrences in the problem, % usageCnt
number of clauses in the problem that contain at least one occurrence of the symbol, % unitUsageCnt
...
indicator of occurrence in a conjecture clause, % inGoal
indicator of occurrence in a unit clause, % inUnit
% Witness: PUZ001-1.p
indicator of being produced during clausification and % introduced
indicator of being produced during skolemization. % skolem
This choice of symbol features is motivated by the fact that they
are readily available in the \gls{atp} \gls{vampire}
and that they suffice as a basis for common precedence generation schemes,
such as the \texttt{invfreq} scheme.

We represent a pair of symbols by the concatenation of their feature vectors.
...
\begin{itemize}
	\item Input: Embedding of a symbol pair \(s_i, s_j\) in problem \(P\)
	\item Target: \({W_P}_{i,j}\) -- an element of the preference matrix we learned for problem \(P\)
\end{itemize}
...
Thanks to how \(W_P\) is constructed (see \autoref{sect:preferences}),
...
\begin{enumerate}
	\item Construct a preference matrix \(W_P\).
	\item Construct a precedence \(\pi_P\) that approximately minimizes
	the proxy cost \(\CostProxy(\pi_P, W_P)\).
\end{enumerate}

\subsubsection{Preference matrix construction}

To construct a preference matrix \(W_P\) for a new problem \(P\),
we evaluate the general preference regressor
on the feature vectors of all symbol pairs in \(P\).

Note that we could use \(W_P\) to estimate the cost of an arbitrary symbol precedence.
Instead of that we will use it to find a symbol precedence
that approximately minimizes the estimated cost.

\subsubsection{Precedence construction from preference matrix} \label{sect:ltot}

The remaining task is, given a preference matrix \(W_P\), to find a precedence \(\pi_P\)
that minimizes \(\CostProxy(\pi_P, W_P)\).
While this task is NP-hard in general,
\citet{Cohen2011} propose a greedy 2-approximation algorithm.
...
\todo[author=Filip]{Should we run an experiment to determine the actual approximation reached on small problems?}
...
The algorithm maintains a partially constructed symbol precedence (initially empty),
a set of available symbols \(\SymbolsAvail\) (initially containing all symbols in \(\symbols\))
and a \emph{potential value} for each of the symbols \(\potential: \SymbolsAvail -> \re\).
It repeatedly selects one of the available symbols,
removes it from the available set and
appends it to the partial precedence.
...
\todo[author=Filip]{Should we explain in more detail?}
...
In each iteration, a symbol with the smallest potential is chosen as the next in the precedence
and removed from \(\SymbolsAvail\)
and the potentials of the remaining symbols in \(\SymbolsAvail\) are updated.
...
\todo[inline,author=Filip]{Update with the final experiment setup and results.}

\subsection{Setup}

Since the ordering strategies under consideration (\acrshort{lpo} and \acrshort{kbo})
...
we use TPTP v7.2.0 \cite{Sut17}, a library of problems.

We repeatedly sample a training set of 100 problems and test set of 1000 problems.
We restrict the sampling to \acrshort{fol} and \acrshort{cnf} problems with at most 1024 predicate symbols for test set and at most 200 predicate symbols for training set.
% problems/predicate-small-1024.txt
We further restrict the training set based on the results of 24 probe runs with uniformly random predicate precedences:
we only consider problems with at least 1 successful run and at least 1 failing run out of the 24 probe runs.
% problems/predicate-1.txt
\todo[author=Filip]{There is noise in the experiment: we have added some problems with more than 1024 predicates.}
We sample the problems uniformly in a way that ensures that the training and test problem sets do not overlap.

On each training problem we run the \gls{atp} Vampire with 100 uniformly random predicate precedences
and with a strategy fixed up to the predicate precedence.\footnote{Time limit: 10 seconds, memory limit: 8192 MB, literal comparison mode: predicate, function symbol precedence: \texttt{invfreq}, saturation algorithm: discount, age-weight ratio: 10, avatar: disabled.}
\todo[author=Filip]{Write the age-weight ratio as a ratio. Is it 10:1 in favor of age?}
\todo[author=Filip]{What is the correct way to type "avatar"?}
...
Note that we use a customized version of Vampire to extract a symbol table from each problem.\footnote{\url{https://github.com/filipbartek/vampire/tree/926154f2}}
% `git reflog` shortens the commit hashes to a prefix of length 8.
\todo[author=Filip]{Should we avoid including my username "filipbartek" to allow double-blind review?}

After we fit a preference matrix to each of the training problems (see \autoref{sect:preferences}),
we create a batch of 1000000 symbol pair feature vectors with target values
to train the general preference regressor (see \autoref{sec:general-preference}).
...
We repeat the whole process 5 times to estimate the stability of the training.
...
\begin{tabular}{lrr}
	...
	\hline
	Best of 10 random & 506.40 & 9.75 \\
	\texttt{invfreq} & 480.60 & 10.52 \\
	Weighting(1,1) & 478.00 & 16.26 \\
	Weighting(0,1) & 477.40 & 16.61 \\
	Weighting(0,0) & 466.00 & 17.81 \\
\end{tabular}
% Source: https://docs.google.com/spreadsheets/d/1HSsC7piUAtWt6uwA9SYOX5vmiF0Ab3Ae4FPyGsDALIg/edit?usp=sharing
\todo[inline,author=Filip]{Update the data with results from a new experiment.}
\todo[inline,author=Filip]{Describe the results.}
\todo[inline,author=Filip]{Consider adding example feature weights.}
