...
\begin{itemize}
	...
	\(\CostBase(\pi)\) is the number of iterations of the saturation loop
	started during the proof search.
	\item If the proof search times out, \(\CostBase(\pi)\) is the maximum number
...
Let \(\CostStd(\pi)\) denote the resulting cost value of permutation \(\pi\)
...
\begin{align*}
\CostProxy(\pi) = \sum_{p, q: \inv{\pi}(p) < \inv{\pi}(q)} \pref(p, q)
\end{align*}
...
While the problem of minimizing \(\CostProxy(\pi)\) given an arbitrary \(\pref\) is NP-hard,
...
\begin{itemize}
	\item Input: Flattened \gls{order-matrix} of a precedence \(\pi\)
	\todo[author=Filip]{Explain in more detail what order matrix is.}
	\item Target: \(\CostStd(\pi)\) --
