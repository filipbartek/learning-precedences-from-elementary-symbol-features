...
\todo[inline,author=Review 3]{I would have like to see at least a short section on related work;
	while it might be true that this is the first attempt to learn symbol
	preferences, existing work for instance in the SAT area seems to be
	quite similar in nature, for instance:
	
	Daniel Selsam, Nikolaj Bjørner: Guiding High-Performance SAT Solvers
	with Unsat-Core Predictions. SAT 2019: 336-353}
...
\todo[inline,author=Review 3]{it feels unnecessary/cumbersome to define the notion of "flattened
	matrices" just to obtain dot products; the dot product of matrices is
	called Frobenius inner product, and could directly be used here}
...
\todo[inline,author=Review 1]{The sentence "For the follow-up steps we assume the preferences ... in a
	meaningful way." is not clear. What do you mean exactly by "a meaningful way"?
	And why do you need to "assume" this? Isn't it the whole point of the
...
\todo[inline,author=Review 3]{the paragraph on "Second-order regression" is quite confusing when
	reading it the first time, since the learning of preference matrices
	is only described later. The paragraph becomes clear after studying
	the whole paper, but it could be improved to allow backtracking-free
	reading.}
...
\begin{itemize}
	...
	\todo[author=Review 1]{Why assign the same value to the precedences for which the proof search
		failed as to the worst-case successful precedence? Especially given the short
		timeout you are using in the experiments, it seems to me that such precedences
		should receive a much higher penalty cost. At least, this is one of the
		parameters that could be tuned in experiments or better justified.}
\end{itemize}
...
\todo[author=Review 1]{The choice of what technique to apply (here Lasso
	regression) is only loosely motivated, which is regrettable but usual with
...
% Witness: PUZ001-1.p
and an indicator of being introduced during clausification. % introduced
...
\todo[inline,author=Review 1]{It is not clear to me what the paragraph between these two pages
	describes. What does weighting the samples mean exactly? Will the samples with
	a bigger weight have more impact on the overall learned model? But are these
	weights not assigned in retrospect, once the model is able to assign good
...
\todo[author=Review 1]{Is there a reason that makes predicate preferences provide results that
	are simpler and easier to interpret? or does the simplification only follows
	from having less symbols to deal with?}
\todo[author=Review 2]{One choice for the experimental evaluation puzzles me though: the
	approach is only applied to the precedence of the *predicate* symbols.
	The precedence for the function symbols is determined by the
	inverse-frequency heuristic. I would like the authors to explain this
	choice.}
\todo[author=Review 1]{I don't see how the half-hearted application of the new technique
	makes the results *easier* to interpret. Can you explain more
	concretely why you only learned precedences for the predicate symbols?
	Is there an expectation that symbol precedence is more important for
	predicate symbols than function symbols?}
...
\todo[inline,author=Review 1]{The two sentences about "splits" (i.e. "In each of 5... to evaluate the
	stability of the training.") is ambiguous. The first and second sentences seem
	to be about different things because "evaluation" is used in the first while
	"experiment" is used in the second, but it makes more sense to me if they are
...
\todo[author=Review 1]{Was the timeout imposed to be so short solely because of the important
	amount of experiments needed or is there any particular value in looking only
	at the easier to solve problems? Intuitively, I would have assumed that you
	would be rather more interested in the hard-to-solve problems.}
...
\todo[inline,author=Review 2]{How well does the model fit? Can you show the error on the test
	data set (for the predicted preferences and maybe also predicted
	runtime)?}
...
\begin{table}[ht]
	...
	\label{table:results}
\end{table}
\todo[inline,author=Review 1]{In table one, it would be more interesting to have the standard deviation
	rather than the raw values per split. (Of course, if there is space, having
	both wouldn't hurt.)}
...
\todo[inline,author=Review 2]{The coefficient table in Sec 5.3 is very interesting. Two
	observations:
	
	- The coefficients in the splits are asymmetric. Is this to be
	expected? Would it make sense to assume a model where the preference
	of (s,t) is the negation of (t,s)?
	
	- Splits 0 and 3 generate a very different preference function. If I
	interpret this correctly, it is another well-known heuristic: the
	"sorted by arity" heuristic, right?}
...
\todo[author=Reivew 1]{A reference is needed regarding graph neural networks.}

\bibliographystyle{plainnat}
\bibliography{main}

\todo[inline,author=Review 1]{Globally, the bibliography should be more uniform. For example, it is
	strange that [10] and [11] have such a different citation style although they
