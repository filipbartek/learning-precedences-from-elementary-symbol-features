...
\todo[inline]{Martin, write one more paragraph here.}
...
To the best of our knowledge, we are the first to try this (high risk high gain)
...
\end{align*}
where we use \(\iverson{P}\) to denote the Iverson bracket \cite{Iverson1962} applied to proposition \(P\),
...
\subsection{General considerations and overview}
...
first evaluating the general preference regressor for all pairs of symbols, 
\todo[inline]{Martin: tady si to zaslouzi rozepsat lepe! 
Ze vytvarime zbrusu novou matici $W_P$, kterou jeste nikdo nevidel.
Ze buchom ji mohli pouzit na predikovani vykonu jednotlivych precedenci,
ale ze mi ted chceme tu nejlepsi, kterou umi.}
and then using a greedy algorithm described in \citet{Cohen2011}
to construct the precedence using these estimates of preference values.
While the problem of finding the optimum \(\pi_P\) given a \(W_P\) is NP-hard in general,
the greedy algorithm is 2-approximation.
In this manner we find a \(\pi_P\) that approximately minimizes \(\CostProxy(\pi_P)\).

\subsubsection{Learning to Order Things} \label{sect:ltot}

\todo[inline]{Martin: Slibil jsem (mini-)sekci, kde se vysvetli LToT.}

\todo[author=Filip]{Too strong claim?}
\todo[author=Martin]{Trochu rozepsat, aby lidi taky meli radost, ze existuje LtOT.}
