...
In this work, we assume that the theorem prover\todo[author=Filip]{Which theorem prover?}{} uses a simplification ordering from one of these two classes
...
% \todo[author=Filip]{I replaced the placeholder "x" with ast(). Is this what you had in mind, Martin?}
...
This is the reason why we (have to) use a representation of symbols by their \emph{features}
when aggregating the learned preferences across different problems (cf. \autoref{sect:embeddings}).
...
\todo[author=Filip]{Don't we need to use the same value of regularization parameter in order for the preference matrix values to be comparable across problems?}
...
\todo[inline,author=Filip]{Discuss: We assume that the overall cost is a sum of contributions of symbol pairs. This assumption may be too strong but we proceed with it anyway.}
...
namely the \texttt{invfreq} scheme.

We represent a pair of symbols by the concatenation of their feature vectors.
\todo[inline,author=Filip]{Make sure that problem embedding is omitted from symbol pair embedding in the experiment.}
...
Experiments show that using sample weighting
...
We restrict the sampling to \acrshort{fol} and \acrshort{cnf} problems with at most 1024 predicate symbols.
...
We sample in a way that ensures that the training and test problem sets do not overlap.

On each training problem we run the \gls{atp} Vampire with 100 uniformly random predicate precedences
and with a strategy fixed up to the predicate precedence.
\todo[author=Filip]{Shall we describe the strategy?}
% Vampire options:
% --literal_comparison_mode predicate --symbol_precedence frequency --saturation_algorithm discount --age_weight_ratio 10 --avatar off --time_limit 10 --memory_limit 8192
