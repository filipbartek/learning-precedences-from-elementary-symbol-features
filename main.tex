...
\subsection{Reduction to preference learning} \label{sect:preferences}

\subsubsection{Problem preference matrix learning}
...
\todo[author=Filip]{Consider using preference matrix instead of preference function.}
...
\(\pref_P : \symbols \times \symbols \rightarrow \re\)
assign a value to each pair of symbols in problem \(P = (\symbols, \clauses)\).
...
Observe that if \(\pref_P\) is represented by an \(n \times n\) matrix \(M_{pref}\),
computing \(\CostProxy(\pi_P, \pref_P)\)
amounts to computing the dot product \(\flatten{O(\pi_P)} \cdot \flatten{M_{pref}}\).
Since for any given problem we can generate a training set consisting of pairs
\((\flatten{O(\pi_P)}, \CostStd(\pi_P))\) for various precedences \(\pi_P\),
we can use linear regression to find a vector \(\flatten{M_{pref}}\)
that minimizes mean square error \(\frac{1}{m} \sum_{\pi_P} (\CostProxy(\pi_P) - \CostStd(\pi_P))^2\).
We use the Lasso regression algorithm \cite{?} to prevent overfitting.

In the following sections we assume that \(M_{pref}\) we find by linear regression gives a good approximation of \(\CostStd\).
...
\subsubsection{Precedence construction from preference function}

When presented with a preference function \(\pref_P\),
we may want to find a precedence \(\pi_P\) that minimizes \(\CostProxy(\pi_P, \pref_P)\).
While the problem of finding the optimum \(\pi_P\) is NP-hard,
...
\subsubsection{General preference learning}

When presented a problem \(P\) we have not encountered during training,
we first construct a preference matrix
\todo[inline]{Finish.}

\subsubsection{Notes}

We define \emph{estimation error}
% Source: https://cw.fel.cvut.cz/b191/_media/courses/be4m33ssu/erm2_ws2019.pdf
...
In the sections that follow we will describe how to find a general function \(\pref\)
that approximates a good \(\pref_P\) for a class of problems.

\subsection{Preference learning as supervised machine learning}

\todo[inline,author=Martin]{If there is time left, restructure the description to bottom-up instead of top-down to make it easier to understand.}

In order to cast the problem of finding an appropriate \(\pref\) function
as a supervised machine learning task,
we need to represent each pair of symbols by a feature vector
...
This choice of symbol features is motivated by the fact that the values of all of these symbol properties
...
\todo[inline,author=Filip]{Fix this statement so that it makes sense.}
\todo[inline,author=Filip]{Discuss in more detail.}
...
\todo{Martin "each of the precedence heuristics" je zbytecne obecny. 
Staci nam poznamenat, ze pres Usage count ``umime'' frequency. Takze by mela jit naucit, nebo neco lepsiho.}
