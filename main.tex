...
\todo{Martin: Hezky!
Asi by to slo dal zlepsit a definovat, co se mysli tim: $\symbols^*$...
Konecna slova nad abecedou? Tam sice mozna intuitivne funguje append, 
ale zas final \(\mathbf{p}\) neni precedence tak, jak ji mame zavedenou.}
...
Inspection of the trained feature weights reveals that the fitting ends up
...
Since Elastic-Net is a linear regression model, we can inspect the weights it assigns
to the input features (see \Cref{sect:embeddings}).
In each of the five splits, the final weights of the three indicator features are 0.
\Cref{table:feature-weights} shows the fitted non-zero weights of the remaining features.
Note that scaling the weights by a constant does not affect
...
\begin{table}[ht]
	...
	\end{tabular}
	% Source: https://docs.google.com/spreadsheets/d/1HSsC7piUAtWt6uwA9SYOX5vmiF0Ab3Ae4FPyGsDALIg/edit?usp=sharing
	\caption{Elastic-Net feature weights after fitting on each of the 5 training sets of 1000 problems each and on the whole \(\ProblemsTrain\).
		...
		The weights are scaled so that their absolute values sum up to 1 in each weight vector.
		Zero-valued weights are omitted.}
	\label{table:feature-weights}
\end{table}

We can namely see that the regressor fitted on the whole \(\ProblemsTrain\)
