...
A precedence \(\pi_P\) \emph{determines}\todo[author=Filip]{Why the emphasis on "determines"?}{} a (total) ordering on \(\Sigma\) as follows:
...
Namely for a symbol precedence \(\pi_P\), \(O(\pi_P)_{i, j} = 1\) if
...
\begin{itemize}
	...
	\(\CostBase(\pi_P) = \max_{\pi_P' \in \Pi_1(P)} \AbstractTime(P, \pi_P')\),
	where \(\Pi_1(P)\) is the set of all training precedences on problem \(P\)
...
\begin{enumerate}
	...
	we first transform the base costs by applying a logarithm.
...
We define \emph{proxy cost of precedence \(\pi_P\) under preference \(\pref_P\)}
...
We consider the preference function \(\pref_P\) perfect if
minimizing the corresponding proxy cost yields a precedence that minimizes the standardized cost:

\begin{align*}
\CostStd \left ( \argmin_{\pi_P} \CostProxy(\pi_P, \pref_P) \right ) = \min_{\pi_P} \CostStd(\pi_P)
\end{align*}

We define loss of preference function \(\pref_P\) on problem \(P\) in the following way:

\begin{align*}
\mathit{loss}(\pref_P, P) = \CostStd \left ( \argmin_{\pi_P} \CostProxy(\pi_P, \pref_P) \right ) - \min_{\pi_P} \CostStd(\pi_P)
\end{align*}

\todo[author=Filip,inline]{Continue writing here. Replace the rest of this section.}

Let the \gls{pairwise-preference-function}
\todo[author=Filip]{Is it ok to use the name "preference" for a function that is to be minimized, thus actually a cost? I adopted it from LtOT terminology. Alternative: pairwise cost.}
\(\pref_P : \Sigma \times \Sigma \rightarrow \re\)
assign a preference value to each pair of symbols in problem \(P = (\Sigma, \clauses)\):
a high value of \(\pref_P(s, t)\) corresponds to a long expected proof search time
if a precedence that orders \(s\) before \(t\) is used.
Furthermore, let the absolute value of \(\pref_P(s, t)\)
correspond to the expected magnitude of impact of the mutual order of \(s\) and \(t\).
Namely, \(\pref_P(s, t) = 0\) means that the mutual order of \(s\) and \(t\)
...
\begin{align*}
\CostProxy(\pi_P) = \sum_{i, j: \inv{\pi_P}(i) < \inv{\pi_P}(j)} \pref_P(s_i, s_j)
= \sum_{i, j} \OrderMatrix(\pi_P)_{i, j} \cdot \pref_P(s_i, s_j)
\end{align*}
...
While the problem of minimizing \(\CostProxy(\pi_P)\) given an arbitrary \(\pref_P\) is NP-hard,
...
\todo[author=Filip]{Add the feature "skolem" to accommodate function symbols.}
